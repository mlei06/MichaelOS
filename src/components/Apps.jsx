...
// <image-slot> is a custom element used in the resume + post hero. Its
// registration script (public/image-slot.js) is loaded as a plain <script>
// in the page shell so it runs only in the browser, not during SSG.
...
function PostApp({ arg }) {
  ...
  const isFull = node.meta && node.meta.full;
  ...
  return (
    <div className="app-content">
      ...
      <div className="prose">
        <p style={{fontSize: 16, color: "var(--text)"}}>{node.meta.desc}</p>
        <div className="rich-content">{renderRichContent(node.content)}</div>
        {!isFull && (
          <p style={{color: "var(--text-dim)", fontStyle: "italic", marginTop: 24}}>
            (Excerpt only — full posts available on request.)
          </p>
        )}
      </div>
    </div>
...
function ContactApp() {
  const items = [
    {
      label: "email",
      href: "mailto:michael.lei@duke.edu",
      svg: (
    ...
    {
      label: "linkedin",
      href: "https://www.linkedin.com/in/michael-lei-1a99b6169",
      svg: (
        <svg viewBox="0 0 24 24" fill="currentColor">
...
const APPS = {
  ...
  resume:   { title: "Resume — resume.pdf",         icon: "↓",  comp: ResumeApp },
